...
class Model(Container):
    ...
    def fit_h5generator(self, generator, nb_epoch,
        ...
        gen_train = generator
        if val_gen:
            gen_val = validation_data

        while epoch < nb_epoch:
            generator.__init__(gen_train.feature_path,
                               gen_train.split,
            ...
            if do_validation:
                if val_gen:
                    validation_data.__init__(gen_val.feature_path,
                                             gen_val.split,
                                             gen_val.batch_size)
                    val_outs = self.evaluate_h5generator(
                        validation_data)
                else:
                    # no need for try/except because
                    # data has already been validated
    ...
    def evaluate_h5generator(self, generator):
        '''Evaluates the model on a data generator. The generator should
        return the same kind of data as accepted by `test_on_batch`.
    ...
    def predict_h5generator(self, generator):
        '''Generates predictions for the input samples from a data generator.
        The generator should return the same kind of data as accepted by
        `predict_on_batch`.
